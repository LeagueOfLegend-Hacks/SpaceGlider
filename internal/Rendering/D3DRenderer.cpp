...
float* D3DRenderer::GetViewMatrix() {
	return (float*)((DWORD)this + 0x54);
}

float* D3DRenderer::GetProjectionMatrix() {
	return (float*)((DWORD)this + 0x94);
}

void D3DRenderer::MultiplyMatrices(float* out, float* a, float* b) {
	int size = 4 * 4;
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			float sum = 0.f;
...
Vector2 D3DRenderer::WorldToScreen(const Vector3& pos) {
