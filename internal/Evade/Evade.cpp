...
{ 
	...
	void Core::OnDraw(LPDIRECT3DDEVICE9 Device)
	{
		GameTimer = Functions::GameTime();
		...
		render.draw_circle(Local->Position, Local->GetBoundingRadius());
	...
	void Core::MoveToPos(Vector3 pos)
	{
		...
		Vector2 path = riot_render->WorldToScreen(pos.Extend(MyHeroPos, -BoundingRadius + 25));
		if (Functions::GameTimeTick() - LastClick > 1000 / 80) {
			LastClick = Functions::GameTimeTick();
			if (Local->Position.distanceTo(pos) > 75)
			{
				Functions::IssueOrder(EOrderType::move, path.x, path.y);
				return;
			}
		}
	}
	...
	void Core::StartEvading()
	{
		...
		if (!Functions::IsAlive(Local) || !Local->IsTargetable || Local->GetAIManager()->Dashing)
		{
			SetEvading(false);
	...
	Vector3 Core::GetExtendedSafePos(Vector3 pos)
	{
		...
		{
			if (Functions::IsAlive(minion) && minion->IsVisible && minion->MaxHealth > 5)
			{
		...
		for (int i = 1; i < 8; i++)
		{
			...
			if (i > 1 && !Functions::IsWall(&ext) || i == 1)
			{
	...
	float Core::GetMovementSpeed(bool extra, EvadeSpell evadeSpell)
	{
		...
		else if (name == "KaisaE")
		{
			...
			return speed[level] * 2 * min(1, Functions::GetAttackDelay(Local)) * moveSpeed;
		}
	...
	float Core::GetDistanceToChampions(Vector3 pos)
	{
		...
		for (GameObject* hero : ObjectManager::HeroList()) {
			if (hero != NULL && hero->IsVisible && Functions::IsAlive(hero) && hero->IsEnemyTo(Local))
			{
	...
	float Core::GetDistanceToMinions(Vector3 pos)
	{
		...
		for (GameObject* minion : ObjectManager::MinionList()) {
			if (minion != NULL && minion->IsVisible && Functions::IsAlive(minion) && minion->MaxHealth > 5)
			{
	...
	float Core::GetDistanceToTurrets(Vector3 pos)
	{
		float minDist = FLT_MAX;
			...
			{
				if (turret != NULL && Functions::IsAlive(turret) && turret->IsEnemyTo(Local))
				{
	...
	{
		float posValue = pos.distanceTo(Functions::GetMouseWorldPosition());
	...
	Vector3 Core::GetBestEvadePos(std::list<Spell> spells, float radius, int mode)
	{
			...
			for (size_t i = 0; i < poly.Points.size(); ++i)
			{
				...
				if (distSqr <= 360000)
				{
					...
					for (int j = -step; j <= step; j++)
						...
						if (!IsDangerous(candidate) && !Functions::IsWall(&candidate))
						{
		...
		if (points.size() > 0)
		{
			...
			Vector3 tempMPos = Functions::GetMouseWorldPosition();
