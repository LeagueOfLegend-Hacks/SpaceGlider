...
float HealthPrediction::GetHealthPrediction(GameObject* unit, int time, int delay)
{
	...
	for (auto attack : ActiveAttacks)
	{
		...
		if (attack.second->target->IsInRange(Local, FLT_MAX, false) && attack.second->target->NetworkID == unit->NetworkID)
		{
			...
			if (landTime < Functions::GameTimeTick() + time)
...
float HealthPrediction::LaneClearHealthPrediction(GameObject* unit, int time, int delay)
{
	...
	{
		int n = 0;
		if (Functions::GameTimeTick() - 100 <= attack.second->startTick + attack.second->animationTime && attack.second->target->IsInRange(Local, FLT_MAX, false) && attack.second->source->IsInRange(Local, FLT_MAX, false) && attack.second->target->NetworkID == unit->NetworkID)
		{
			float fromT = attack.second->startTick;
			float toT = Functions::GameTimeTick() + time;

			while (fromT < toT)
			{
				if (fromT >= Functions::GameTimeTick() && (fromT + attack.second->delay + max(0, unit->Position.Distance(attack.second->source->Position) - attack.second->source->GetBoundingRadius() / 2) / attack.second->projectileSpeed < toT))
					n++;
...
void HealthPrediction::OnGameUpdate()
{
	...
	{
		if (pair.second->startTick < Functions::GameTimeTick() - 3000)
...
void HealthPrediction::OnProcessSpell(void* spellBook, SpellInfo* castInfo) {
	...
	if (caster->Team != Local->Team || Helper::isValidUnit(caster, 3000, false) || (!Functions::IsMinion(caster) || !Functions::IsTurret(caster)))
		return;
	...
	//console.Print("%f", Functions::GetBasicAttack(Local)->Resource->MissileSpeed);

	auto data = new PredictedDamage(
		caster, 
		target, 
		caster->Position, 
		Functions::GameTimeTick() - Functions::GetPing() / 2, 
		Functions::GetAttackCastDelay(caster) * 1000, 
		Functions::GetAttackDelay(caster) * 1000 + (Functions::IsTurret(caster) ? 70 : 0), 
	...
	//console.Print("[%p, \n%p, \n%f, \n%f, \n%f, \n%f, \n%f]", caster, target, Functions::GameTimeTick() - Functions::GetPing() / 2, Functions::GetAttackCastDelay(caster) * 1000, Functions::GetAttackDelay(caster) * 1000 - (Functions::IsTurret(caster) ? 70 : 0), caster->IsMelee() ? FLT_MAX : castInfo->BasicAttackSpellData->Resource->MissileSpeed, (float)caster->BaseAttackDamage + caster->BonusAttackDamage);
